...
export default function Menu(props: { buttonClassName?: string }) {
  ...
  const { room, setIsBackgroundSelectionOpen, setIsParticipantWindowOpen } = useVideoContext();

  const anchorRef = useRef<HTMLButtonElement>(null);
  const { flipCameraDisabled, toggleFacingMode, flipCameraSupported } = useFlipCameraToggle();

  const recordingVideo = async () => {
    await videoSessionService.recordingVideo(room!.sid, isRecording);
  };
  ...
  return (
    <>
      ...
      >
        ...
        {isSupported && (
          <MenuItem
            onClick={() => {
              ...
              setIsChatWindowOpen(false);
              setMenuOpen(false);
              setIsParticipantWindowOpen(false);
            }}
          >
            <IconContainer>
              <BackgroundIcon />
        ...
        <MenuItem onClick={handlePeopleClick}>
          <IconContainer>
            <BackgroundIcon />
          </IconContainer>
